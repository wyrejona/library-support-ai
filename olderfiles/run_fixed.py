...
def check_dependencies():
    ...
    required = [
        ...
        'sqlalchemy',
        'pydantic',
        'sentence_transformers',  # Module name with underscore
        'ollama',
        'pypdf',
    ]
    ...
    for package in required:
        try:
            __import__(package)  # FIXED: No replace needed
            print(f"  ✅ {package}")
        except ImportError:
            print(f"  ❌ {package}")
    ...
    if missing:
        ...
        # Show pip names for installation
        pip_names = [p.replace('_', '-') for p in missing]
        print("   Install with: pip install " + " ".join(pip_names))
        return False
